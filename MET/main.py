...
dataset = MNetDataset("../dataset/MET", n_train=args.n_train)
LABEL = dataset.LABEL
...
if args.model == "umt":
    model = UMT(sentence_encoder=sentence_encoder, n_label=len(LABEL), use_cf=args.cf)
elif args.model == "mkgformer":
    model = MKGformer(
        sentence_encoder=sentence_encoder, n_label=len(LABEL), use_cf=args.cf
    )
...
for _ in range(args.epoch):
    ...
    if val_result["micro_f1"] >= max_f1:
        ...
        model.save_weights(
            f"./weights/met-{args.model}-cf-{args.cf}-n-{args.n_train}.h5"
        )
