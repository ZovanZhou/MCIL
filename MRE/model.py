...
class MKGformer(tf.keras.models.Model):
    ...
    def mm_fusion(self, data, training, stop_gd: bool = False):
        ...
        h_ph = self.get_tensor_by_index(h_mm, batch_size, ph)
        h_pt = self.get_tensor_by_index(h_mm, batch_size, pt)
        h_cls = h_mm[:, 0, :]
        h_e = tf.concat([h_ph, h_pt, h_cls], axis=-1)
        if training:
